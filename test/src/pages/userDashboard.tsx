      ...
      {editing && (
        <div className="mt-10 max-w-4xl mx-auto shadow-lg rounded-2xl p-8 w-full bg-white/20 backdrop-blur-lg border border-white/30">
          ...
          <div className="flex flex-col items-center mb-6">
            <img
              src={
                typeof editing.photo === "string"
                  ? editing.photo
                  : editing.photo
                  ? URL.createObjectURL(editing.photo)
                  : "/default-avatar.png"
              }
              alt={editing.name}
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col">
              <label className="font-medium mb-1 text-gray-700">Photo</label>
              <input
                type="file"
                onChange={(e) => {
                  if (e.target.files && e.target.files[0]) {
                    setEditing({ ...editing, photo: e.target.files[0] } as User);
                  }
                }}
                className="border border-gray-300 p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500"
              />
